...
namespace ttp {
    ...
    TSP ProblemFactory::createTSPFromFile(std::string pathToFile) {
        LKHWrapper lkh;
...
/*
    ...
    TTP ProblemFactory::createTTP(TSP tsp, KnapsackProblem knp, int itemsPerCity) {
        ThiefMapPtr ptr(tsp.getMap());
        ptr->insert(knp.getItems().begin(), knp.getItems().end());
        TTP ttp(ptr, knp.getMaxWeight());
        return ttp;
    }
